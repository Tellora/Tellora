    ...
    return (
        <section className="relative z-20 -mt-12 md:-mt-16 mb-24 pointer-events-none">
            <div className="container mx-auto px-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 pointer-events-auto">
                    {stats.map((stat, i) => (
                        <motion.div
                            key={i}
                            ...
                            className="bg-white p-8 md:p-10 brutalist-border shadow-[6px_6px_0px_#000] hover:shadow-[10px_10px_0px_#000] group/stat hover:-translate-y-1 transition-all"
                        >
                            <div className="flex items-center justify-between mb-4">
                                <div className="text-black opacity-30 group-hover/stat:opacity-100 transition-opacity">
                                    {i === 0 ? <TrendingUp size={20} /> : i === 1 ? <Users size={20} /> : <Target size={20} />}
                                </div>
                                <div className="p-2 bg-primary/5 rounded-lg group-hover/stat:bg-primary/10 transition-colors">
                                    <Zap size={14} className="text-primary fill-current" />
                                </div>
                            </div>
                            
                            <h4 
                                className="text-4xl md:text-5xl font-heading font-black mb-2 tracking-tighter"
                                style={{ color: stat.color }}
                            >
                                {stat.value}
                            </h4>
                            <p className="font-black text-[10px] uppercase tracking-[0.3em] text-black/40 group-hover/stat:text-black transition-colors">{stat.label}</p>
